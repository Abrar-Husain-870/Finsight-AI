...
const variantStyles = {
  standard: {
    ...
    description: 'max-w-xl text-sm sm:text-base leading-relaxed',
    header: 'gap-5',
      ...
      {images.slice(0, 3).map((src, i) => {
        ...
        return (
          <motion.div
            ...
            variants={fanCard}
            className={cn(
              'relative shrink-0 overflow-hidden rounded-2xl shadow-2xl border border-[var(--color-border-primary)] bg-[var(--color-bg-secondary)]',
              cardAspect,
              slot.width,
              slot.layout,
            )}
...
export function Hero10({
  ...
  const titleElement = title && (
    <h2
      className={cn(
        'text-[var(--color-text-primary)] font-bold tracking-tight text-balance',
        vs.title,
      )}
    >
      ...
      {(titleLine2Prefix || titleHighlight) && (
        <>
          <br />
          <Balancer>
            {titleLine2Prefix && <span className="font-normal text-[var(--color-text-secondary)]">{titleLine2Prefix} </span>}
            {titleHighlight && (
              <span className="text-[var(--color-text-primary)] font-extrabold">{titleHighlight}</span>
            )}
          </Balancer>
        </>
      )}
  ...
  const descriptionElement = description && (
    <p className={cn('text-[var(--color-text-secondary)] font-normal', vs.description)}>
  ...
  const socialProofElement = socialProof && (
    <p className="text-[var(--color-text-secondary)] text-xs font-semibold uppercase tracking-wider bg-[var(--color-bg-secondary)] px-3 py-1 rounded-full border border-[var(--color-border-primary)]">
      {socialProof}
    </p>
  )
  ...
  return (
    <section className={cn("relative isolate w-full overflow-hidden bg-[var(--color-bg-primary)] text-[var(--color-text-primary)]", className)}>
      ...
      >
        ...
        <div className={cn('mx-auto w-full pt-4', vs.fan)}>{mediaElement}</div>
      </motion.div>
    </section>
  )
}
